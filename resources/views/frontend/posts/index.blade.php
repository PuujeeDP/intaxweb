    ...
    <section class="relative overflow-hidden"
        ...
        <div class="absolute inset-0 opacity-10">
            <svg class="w-full h-full" xmlns="http://www.w3.org/2000/svg">
                <defs>
                    <pattern id="hexagon-pattern-posts" x="0" y="0" width="56" height="100" patternUnits="userSpaceOnUse">
                ...
                <rect width="100%" height="100%" fill="url(#hexagon-pattern-posts)" />
            </svg>
        </div>
        ...
        <div class="absolute inset-0 overflow-hidden opacity-15">
            ...
            <div class="absolute bottom-0 right-0 w-96 h-96 bg-red-300 rounded-full filter blur-3xl animate-pulse"
            ...
            <div class="text-center">
                <h1 class="text-4xl md:text-5xl lg:text-6xl font-extrabold text-white mb-6 leading-tight"
                    ...
                    @if ($search)
                        ...
                        <span
                            class="bg-gradient-to-r from-red-200 to-red-100 bg-clip-text text-transparent">"{{ $search }}"</span>
                    @else
                        {{ __('frontend.blog_posts') }}
                    @endif
                </h1>
                                    ...
                                    @foreach ($categories as $category)
                                        <li>
                                            <a href="{{ localized_route('categories.show', ['slug' => $category->slug]) }}"
                                                class="flex items-center justify-between px-4 py-2.5 rounded-lg hover:bg-[#d40c19]/5 text-gray-700 hover:text-[#d40c19] transition-all duration-300 group">
                                                <span class="font-medium">{{ $category->name }}</span>
                                                @if ($category->posts_count > 0)
                                                    <span
                                                        class="px-2.5 py-0.5 text-xs rounded-full bg-gray-100 text-gray-600 group-hover:bg-[#d40c19] group-hover:text-white font-semibold transition-all">
                            ...
                            <div class="bg-white rounded-2xl shadow-md p-6 border border-gray-100">
                                ...
                                <ul class="space-y-4">
                                    @foreach ($posts->take(5) as $recentPost)
                                        <li>
                                            <a href="{{ localized_route('posts.show', ['slug' => $recentPost->slug]) }}"
                                                class="flex items-start gap-3 group">
                                                @if ($recentPost->featuredImage)
                                                    <img src="{{ $recentPost->featuredImage->url }}"
                                                ...
                                                <div class="flex-1 min-w-0">
                                                    ...
                                                    <time
                                                        class="text-xs text-gray-500 mt-1 block">{{ $recentPost->published_at?->format('Y.m.d') }}</time>
                                                </div>
                                            </a>
                                        </li>
                                    @endforeach
