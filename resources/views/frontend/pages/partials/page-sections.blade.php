@if ($page->sections && $page->sections->count() > 0)
            ...
            <div class="flex flex-wrap gap-2 mb-8 border-b-2 border-gray-200">
                @foreach ($page->sections as $index => $section)
                    <button @click="activeTab = {{ $index }}"
                        ...
                        <span x-show="activeTab === {{ $index }}"
                              class="absolute -bottom-2 left-1/2 -translate-x-1/2 w-0 h-0 border-l-[8px] border-l-transparent border-r-[8px] border-r-transparent border-t-[8px] border-t-green-600"
                              x-transition:enter="transition ease-out duration-300"
            ...
            @foreach ($page->sections as $index => $section)
                <div x-show="activeTab === {{ $index }}"
                    ...
                    <div class="prose prose-lg prose-green max-w-none
                                ...
                                prose-ul:list-disc prose-ul:ml-6 prose-ul:space-y-2 prose-ul:marker:text-green-600
                                prose-ol:list-decimal prose-ol:ml-6 prose-ol:space-y-2 prose-ol:marker:text-green-600
    ...
    @else
        ...
        <div x-data="{ openAccordion: null }" class="mt-12 space-y-4 animate-fade-in-up" style="animation-delay: 0.5s;">
            @foreach ($page->sections as $index => $section)
                <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-100 overflow-hidden transition-all duration-300 hover:shadow-xl">
                    <button
                        ...
                        <svg class="w-6 h-6 transform transition-transform duration-300 flex-shrink-0"
                            :class="openAccordion === {{ $index }} ? 'rotate-180 text-green-600' : 'text-gray-400'"
